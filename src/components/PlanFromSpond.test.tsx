...
describe('PlanFromSpondView', () => {
  ...
  it('freezes every plan control while an outer write is in flight', () => {
    // On the planner, this surface sits above a draft that is itself being
    // saved; planning an event would abandon that draft, so frozen holds every
    // Plan this closed even though no create of its own is running.
    const html = render({
      rows: [ev({ id: 'e1' }), ev({ id: 'e2', title: 'U8 Match' })],
      frozen: true,
    })
