// Ported from the prototype data.js. This is the Phase 1 stand in for the
// server data layer: the screens read from these typed arrays exactly as the
// prototype read from window.OTJ. Phase 2 replaces these reads with TanStack
// Query hooks against Supabase, at which point this module is removed. The
// shapes mirror the prototype (camelCase); the database uses snake_case and
// the mapping is introduced in Phase 2.
...
export const mediaById: Record<string, MediaItem> = Object.fromEntries(media.map((m) => [m.id, m]))
...
export const drillById: Record<string, Drill> = Object.fromEntries(drills.map((d) => [d.id, d]))
