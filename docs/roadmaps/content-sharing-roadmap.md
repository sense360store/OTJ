...
- Non scope: copy and import (PR 7). Delivered combined with the browser-print portion of PR 6, per the PR 6 section's own allowance; the combined PR inherits PR 4's gate (auto merge prohibited).
- Attached PDF policy as built: an `internal_only`, missing or cross club PDF blocks the WHOLE programme share rather than being silently omitted, and a programme with no weeks at all is refused. Both rules are stated in the boundary doc (section 34) and pinned by tests.
- Expiry as built: unchanged at 90 days for a `shares.create` holder. The 180 day programme default floated in section 8 and decision 7 is NOT implemented; it would require changing the SQL expiry policy, which is its own decision.
...
- Migrations: one small gated migration (`0041`) was required, correcting this roadmap's earlier "none" estimate, exactly as PR 3 corrected its own. Every programme-shaped piece of the PR 1 substrate already existed and was already live (the `programme` enum value, `content_shares.programme_id` and both check constraints, the one-active and idempotency indexes, the `content_share_deps` programme branch from `0040`, the lifecycle RPC's programme arms, the `programmes.create` and `programmes.manage` capabilities, the audit vocabulary, and the downgrade triggers). The one blocker was the anonymous read path: `read_public_share` failed closed for any kind other than drill or session. `0041` widens that allow list to {drill, session, programme} and nothing else; outside comments its body differs from `0040` in exactly one line, and the signature is unchanged. RPCs: reuse. Edge Functions: extend the builder, redeploy with readback.
- Capability changes: none. Audit actions: reuse.
