...
| BUG-01 | Product quality | A session created by hand defaulted to a fixed date in the past | In progress | P1 | Found by a production smoke test on 9 September 2026: a new session was dated 16 June. `blankSession` and the Use template path each carried the literal `'2026-06-16'`, so one defect was written twice and a check on either proved nothing about the other. Fixed in #236 alongside COACH-14A, which is the slice already touching both: one derivation (`src/lib/localDate.ts`) with three consumers, reading local calendar fields rather than a UTC string slice, because `toISOString().slice(0, 10)` dates a session created after midnight in British Summer Time to yesterday. Plan from Spond and Apply programme derive their dates from an event and from an admin's start date and are unchanged, which is asserted rather than assumed. Migration free; no stored date is rewritten |
| COACH-14 | Coaching workflow / Planning | Guided Session Builder and Quick drill creation: plan a whole session from Plan a session without first learning where OTJ keeps drills, week plans and programmes | In progress | P1 | `docs/roadmap/coach-14-guided-session-builder.md`. Migration free and additive: a guided path OVER the canonical Session model, never a second one. COACH-14A, the foundation, is the entry choice, the four step flow (session basics, coaching focus, session shape, build the activities) and the existing composer at the last step; it reuses the COACH-10 activity editor, the COACH-11 Drill Maker round trip and the one guarded submit, and adds no migration, Edge Function, schema or public projection. Quick drill (COACH-14B), the review step (COACH-14C) and the PDF and share actions (COACH-14D) follow. VISUAL-03 adopts the guided surface as it lands rather than styling it afterwards |
...
6. Land **COACH-14**, the Guided Session Builder, in reviewable slices: 14A the foundation, then Quick drill, the review step and the PDF and share actions. It fixes the entry point into planning itself, so it comes before the deeper operational coaching work (COACH-6, COACH-7, COACH-8) unless production evidence changes the order.
7. Finish with **VISUAL-04**, a whole-product phone-first visual/accessibility QA pass.
