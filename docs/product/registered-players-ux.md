...
**Row actions.** For `players.manage` holders: an "Edit" ghost button, a "History" ghost button (History requires `audit.view`, which managers and admins hold), and an overflow menu (icon button, `aria-label` "More actions for {name}") containing "Move team", "Withdraw" or "Restore" (whichever applies to the row's status), and, for `players.delete` holders only, "Delete permanently". A `players.view` only viewer (a coach) gets no row actions: per player History requires `audit.view`, which a coach does not hold by default. On an archived season only History remains, for `audit.view` holders.
...
The "History" row action opens a Modal (the repo's only overlay primitive; a side drawer is a possible later refinement, Alternatives below), title the player's name, sub "History". Access requires `audit.view` (managers and admins by default) through the dedicated per player history path (`player_history`); a coach holding `players.view` only does not see History by default (docs/security/app-audit-boundary.md; decision 15 resolved to `audit.view`, the safer direction). A child linked history row is pseudonymous child personal data, so this gate is a real access control: requiring `audit.view` keeps historical child linked audit records off the coach surface by default.
...
- 15. Club wide `audit.view` versus a separate per player history path: resolved to `audit.view` (the safer direction); the History row action and the Activity page both require `audit.view` (managers and admins), and a coach holding `players.view` only sees neither by default.
