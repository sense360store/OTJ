...
Imports target the current season only: the `import_players` RPC refuses any other season, so the two import buttons render only when the selected season is the current one (docs/product/registered-players-import-export.md, Season column rules). On a non current, unarchived season (one created but not yet activated, a normal state while the club prepares next season) the import buttons are absent and a muted line under the header actions explains: "Imports go into the current season. Switch to 2026/27 to import." Manual writes are deliberately looser: the database refuses registration writes only on archived seasons, so Add player, the row actions and bulk actions stay available on a future season, gated by capability as usual. Download template is season neutral and renders for `players.import` holders on any season. Export and Download template remain available on an archived season, because neither mutates a record: the export's only write is its own audit event, and the season renewal round trip in docs/product/registered-players-import-export.md depends on exporting a past season, which a club may have archived. `export_players` is bounded to the selected season, current or not; exporting another season means switching the season selector first (docs/product/registered-players-import-export.md).
...
- The "Unassigned" option renders only for `all_teams` holders. Under the recommended team scope (unresolved decision 2) a viewer without `all_teams` can never read an Unassigned registration, so for them the option could only ever produce a misleading empty list; it is therefore absent from their team select. The board picker's team selector follows the same rule: its Unassigned choice renders only for `all_teams` holders (eligibility rules in docs/product/registered-players-spec.md, Unassigned players).
...
| Empty season | Season loaded, zero registrations, viewer holds `all_teams` | `Empty` primitive, title "No players in 2026/27 yet", body "Add the first player, or import a spreadsheet to bring the register across.", with "Add player" and "Import players" buttons rendered per capability (Import players only when the season is current, per section 2); a `players.view` only viewer sees the title and body alone |
| Empty scope | Zero rows returned and the viewer lacks `all_teams` | `Empty`, title "No players on your teams yet", body "You see players on teams you are assigned to. If a team is missing, ask an admin to add you to it." The client chooses this variant purely on the viewer lacking `all_teams`: under the recommended team scope RLS returns zero rows whether the season is genuinely empty or merely out of the viewer's scope, so the scoped copy is used whenever the flag is absent, never the untrue "No players in 2026/27 yet". CTAs render per capability as in the Empty season state (none under the recommended coach reduction) |
...
- Team: select, defaulting to the page's current team filter when a specific team is selected, otherwise "Unassigned". Options are scoped to the writer: an `all_teams` holder (managers and admins, the only default writers) sees Unassigned plus every team; a `players.manage` holder without `all_teams` sees only the teams in their `member_teams` set, with no Unassigned option, because the registration write policies refuse an Unassigned or out of scope team for a scoped writer (docs/security/registered-players-boundary.md) and the select must never offer a choice the save would refuse. For a scoped writer the default falls back to their first team by name when the page filter does not select one of their teams.
...
**Move team.** The overflow action opens a small modal, title "Move team", sub the player's name, a single team select scoped exactly as in Add player (Unassigned plus every team for `all_teams` holders; only the writer's `member_teams` teams otherwise), footer Cancel and "Move" ("Moving…"). Also achievable through Edit; the dedicated action exists because it is the most common correction.
...
- Entering locked mode must never strand focus: the pressed confirm button becomes disabled and the X disappears, which in most browsers would drop focus to the document body, defeating the trap exactly when it matters most. On entering locked mode, focus moves to the dialog container (`tabIndex={-1}`), so the trap always holds a valid target while every control is disabled.
...
**Entry.** "Import players" opens a wide Modal, title "Import players", sub "Into 2026/27". The button renders only when the page's selected season is the current season (section 2), so the sub always names the current season, and the `import_players` RPC revalidates that independently and refuses any other target (docs/product/registered-players-import-export.md). On an archived season, or on a created but not yet activated season, the button never rendered (sections 2 and 4).
...
- A summary sentence built from the authoritative five category partition (every data row is exactly one of new, update, already present, held back, invalid; the five counts sum to the total, as fixed in docs/product/registered-players-import-export.md): "18 rows: 12 new, 3 updates, 1 already present, 1 held back, 1 invalid." Warnings are an overlay on those rows, not a sixth category, so they are stated separately: "2 of these carry warnings." Only non zero categories are mentioned. Blank rows are noted separately when skipped: "2 blank rows skipped."
...
**Import preview and result.** The preview summary is plain text, not an inference from colour; each row's problem is a text detail line adjacent to the name. When parsing completes, an `aria-live="polite"` region announces the summary sentence, built from the same five category partition ("Preview ready. 18 rows: 12 new, 3 updates, 1 already present, 1 held back, 1 invalid. 2 carry warnings."). When the commit completes, the same region announces the outcome sentence ("Imported into 2026/27: 12 added, 3 updated, 1 already present, 1 skipped, 1 rejected."). Failures render in a `role="alert"` block so they announce immediately. Inline save failures in the manual modals also use `role="alert"`.
