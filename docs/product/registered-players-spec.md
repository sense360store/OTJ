...
|---|---|---|
| View, search, filter, sort players | `players.view` | Coach, manager and admin: club wide read (team is a filter, not an access boundary; see Team scope and read access). |
...
| Club wide Activity page; per player History | `audit.view` | Club wide feed and per player history. Managers and admins by default; coaches (`players.view` only) do not see per player history by default (decision 15, resolved to `audit.view`). |
...
- **History (per player).** A panel on the player row showing date and time, actor, action, safe changed fields, source, and the import batch reference where one applies. Example entries: "16 Jul, 14:32 / Mark Taylor / Registration changed: Pending to Registered"; "15 Jul, 10:18 / Neil McRae / Team changed: Unassigned to U8 Titans"; "14 Jul, 09:05 / CSV import / Registration created". Distinct actions back these entries: `player.created` for a new identity, `player.registration_created` for a season registration, then `player.status_changed`, `player.team_changed` and the rest (docs/security/app-audit-boundary.md). Names of actors, teams and players resolve at read time; after a permanent deletion the display shows a neutral identifier (Deleted player, Deleted team). Access requires `audit.view` (managers and admins by default), through a dedicated read path (`player_history`); coaches, holding `players.view` only, do not see per player history by default. A child linked history row is pseudonymous child personal data (docs/security/app-audit-boundary.md), so this gate is a real access control: requiring `audit.view` keeps historical child linked audit records from being exposed to coaches by default, and the trade off (coaches do not see per player history by default) is accepted for that safety. This is decision 15, resolved to `audit.view` (the safer direction). The per player History UI is not built in PR 2; only the gated read path exists.
...
| 15 | `audit.view` versus per player history access | Resolved to `audit.view`: per player History and the club wide Activity both require `audit.view`; coaches (`players.view` only) do not see per player history by default (the safer direction) |
