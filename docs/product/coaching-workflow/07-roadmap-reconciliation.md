...
6. **COACH-1** (M1), the first gated coaching migration, timed against the
   ledger. **In progress**: COACH-1A registered and in review, COACH-1B to
   follow.
...
- **COACH-1 is in progress** since the same day: its database half, COACH-1A,
  ...
  opened as its own gated pull request; COACH-1B, the reorder affordance,
