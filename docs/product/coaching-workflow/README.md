...
| **COACH-5**, venue layouts scoped to venue, season and age group | **Built, awaiting its migration's apply.** Migration `0053_venue_layouts` (M2: `venue_layouts`, the shape predicates and the check constraint, two policies mirroring `venues`, explicit grants, the `audit_venue_layouts` trigger, and `clubs.age_groups`, the club level age group vocabulary the scope key rests on), registered against the hosted head `20260904174142` / `atomic_team_order` read live on 7 September 2026. `src/lib/venueLayout.ts` holds the model, the fail closed season resolution and the five no-layout states; `src/lib/ageGroups.ts` the one age group list; `src/components/VenueLayoutPitch.tsx` the read only drawing and the editor; `/admin/venues/:venueId/layouts` the admin surface, reached from the Venues screen, which is adopted onto the shared system with it; the Club screen edits the age group list and the planner and the programme modal read it. COACH-6 mounts the drawing on session day and is not part of this. |
