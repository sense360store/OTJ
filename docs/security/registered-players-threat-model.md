...
- Mitigation: the RPC re-checks `has_perm('players.export')` server side, applies the club scope to the returned rows (read is club wide, no team arm), and writes the `players.exported` audit event in the same transaction as the read. `players.export` is a separate capability from `players.import` because the risks differ (export exfiltrates names; import mutates records).
...
- Residual risk: a re-import under a fresh batch id degrades safely through the matching rules. An exported file carries Player IDs, so its rows match deterministically by Player ID and update in place with the same values (idempotent). A hand made file without Player IDs never auto merges: a name collision becomes "needs your choice" and the user picks Skip or Import as new, so a fresh batch is never silently classified "already present" from a name.
...
- Residual risk: registrations made Unassigned by a team deletion remain visible to every viewer, because read is club wide (team is a filter, not an access boundary), so the coaches who reassign them can see them. No player vanishes from view on team deletion.
...
- Mitigation: audit rows never contain player names by construction. `safe_changes` holds old and new values only for the approved safe field list (team_id, status, shirt_number, registered_date, season_id); a display name change records `changed_fields = ['display_name']` with no values, rendered as "Player name corrected". `player.deleted` is raised by the AFTER DELETE trigger on the identity row and commits atomically with the deletion in the same transaction, retaining the entity id and no name (the action mapping in docs/security/app-audit-boundary.md); the UI shows a neutral "Deleted player" tombstone. Metadata is restricted to safe scalar facts (counts, format, filter summary).
- Residual risk: a child linked audit row is pseudonymous child personal data (a stable player_id plus team, status and date history), not merely operational data (docs/security/app-audit-boundary.md, Data classification). It names nobody, but the linkage and attribute history are personal data, so a subject erasure request must address the child linked audit identifiers and `safe_changes` and backups explicitly, not rely on the players deletion path alone. Retention and erasure treat these rows as child data (Unresolved items 7 and 8). The classification, not just the name free design, is the control.
...
- Residual risk: upstream schema drift can break the import (it fails with an error or a warning), but it cannot widen what is persisted. A separate, deliberate limitation: because Spond member ids are never persisted, the name only dedupe treats two different children with the same name in one subgroup as one (the second is skipped). This is a documented, accepted trade off of the child data boundary, pinned by a test; the manual add or the id keyed spreadsheet import represents genuine namesakes (docs/product/registered-players-import-export.md).
...
- Mitigation: capability minimisation is the primary control: parents hold nothing, coaches default to `players.view` (club wide read, no write), export and import sit with managers and admins and remain separately grantable. Every export writes a `players.exported` audit event (count, format, season, safe filter summary) in the same transaction as the read, the export confirmation states that the file names children and must be stored and shared securely, and `audit.view` lets the club review who exported what and when.
...
3. Fixtures. Add a `manager` fixture user (club A, manager system role) for export and import capability cells, and a `coachView` fixture holding only `players.view` to prove coaches read the whole club register but cannot write. There is no team scoped fixture, because read and write are club wide (team is a filter, not an access boundary), so no in body team scope arm exists to test. A namesake fixture pair lands in PR 5: two distinct player identities with identical display names on the same team and season, to prove the import never collapses them (a no id file holds both as needs your choice; an id keyed file updates both by id). The signup.test.ts quarantined read loop grows in step with the schema, never ahead of it: `audit_events` joins the loop in PR 1, `player_registrations` and `seasons` in PR 2, and `import_batches` in PR 5, so the loop only ever selects from tables that exist at that phase.
...
| 21 | One current season per club | seasons.test.ts (second `is_current` row refused 23505 by the partial unique index even via serviceClient, the below RLS boundary; the three guard trigger cells: a direct update clearing `is_current` refused P0001, an update setting `archived_at` on the current season refused P0001, a delete of the current season refused, at the grant for clients and P0001 via serviceClient; `activate_season` as admin swaps atomically leaving exactly one; coach call refused) | PR 2 |
...
**tests/security/audit.test.ts** (new in PR 1, extended in PR 2 and PR 8). PR 1 cells: select matrix (admin with `audit.view` reads club rows; parent, capability-less coach, anon and outsider read zero; outsider with club B `audit.view` reads zero club A rows); write matrix (insert 42501 for every JWT; update and delete 42501, grant level, a documented deviation from the zero rows convention because the table carries SELECT only grants; `log_audit_event` EXECUTE refusal 42501 for coach and parent). PR 2 cells: properties 11, 12, 13 and 25 above, plus a name discipline cell: rename a fixture player and assert the event has `changed_fields = ['display_name']` and no name value in `safe_changes` or `metadata` (string containment check over the row JSON, the boards.test.ts payload technique). PR 2 also carries the `player_history` cells (the separately gated read path of decision 15, resolved to `audit.view`, contract in docs/security/registered-players-boundary.md section 4): `admin` and `manager`, holding `audit.view`, read a player's events; `coachOne` and `coachTwo`, holding `players.view` and no `audit.view`, plus `parent` and `outsider`, receive the RPC's clean capability refusal (42501); returned rows contain no name values (the same string containment check); and a club scope cell asserting an outsider holding `audit.view` in club B still reads zero for a club A player. The per player History UI is deferred (not built in PR 2); only the gated database read path exists. PR 8 adds one success event cell per newly audited entity family.

**tests/security/seasons.test.ts** (PR 2). Write matrix (create, update, archive require `seasons.manage`: admin passes, manager, coach and parent refused 42501); properties 20 and 21; the three seasons guard trigger refusal cells required by docs/adr/ADR-0005-registered-players-and-seasons.md: a direct update clearing `is_current` outside `activate_season` refused P0001; an update setting `archived_at` while `is_current` is true refused P0001; deleting the current season refused, for clients at the grant (`seasons` carries no delete grant per docs/security/registered-players-boundary.md) and via serviceClient by the trigger with P0001, proving the invariant holds below RLS; `activate_season` writes exactly one `season.activated` event; date order check constraint 23514 via serviceClient. The seasons read policy follows docs/security/registered-players-boundary.md; the file pins whatever that contract says with one cell per role.

**tests/security/players.test.ts** (rewritten in PR 2). The existing seven cells re-pointed at the new contract: read requires `players.view` (parent still `[]`, outsider still `[]`), writes require `players.manage`, insert pins `created_by = auth.uid()`. New cells for the club wide read model: `coachView` (holding only `players.view`) reads the whole club register including Unassigned but cannot insert, update or delete; manager and admin read all and write; there is no team scoped cell because there is no team arm. The header cites the provisional migrations (0030 onward, live ledger confirmed at apply time) and records that read is club wide, preserving the 0016 standing rule.

**tests/security/player-registrations.test.ts** (new in PR 2). Full role by operation matrix under the club wide capability model (players.view reads all, players.manage writes, no team arm); forged `club_id` and forged `created_by` inserts (properties 8 and 9); status vocabulary check constraint 23514; `UNIQUE (player_id, season_id)` 23505; team deletion turns `team_id` null without deleting the row (the cascade fix, service role fixture); curator profile deletion leaves rows with `created_by` null (the remove-user fix); status transition rules refused server side (a disallowed transition raises P0001).
...
**tests/security/import.test.ts** (PR 5). Properties 5, 7, 8, 13 (import arm), 14, 15, 16, 17 (server arm), 19; season validation (an archived or foreign season refused, and import into any non archived club season permitted, not only the current one); the namesake cells: a no id file naming two distinct children with the same name on the same team holds both as needs your choice and, resolved to Import as new, creates two distinct identities; an id keyed file naming both updates both by id and merges neither; a failed batch rolls back all business and per row audit writes, records one `players.import_failed`, and a replay returns the stored failure; permission revocation cell (serviceClient removes `players.import` from the fixture role between calls, next call refused, T11); batch id cross club replay refused (T14); `import_batches` readable only per the boundary contract, carrying no file fingerprint.

**tests/security/renew.test.ts** (PR 6). The `renew_registrations` RPC per docs/roadmaps/registered-players-delivery-plan.md PR 6: the capability gate (`players.manage` required: manager passes; parent, coach and outsider refused with no rows written); a source season outside the club refused; renewal into any non archived target season permitted; there is no team scope arm to test (read and write are club wide); idempotency per (player, season) on a repeated call; audit events carrying source `renewal` and one shared batch id, committed with the copied rows, with the registration insert raising `player.renewed`.
...
|---|---|
| PR 1 | audit.test.ts (properties 3, 10, 24; 6 audit arm); capabilities.test.ts catalogue and regex extension; CI wiring for `test:security`; matrix doc rows; signup loop addition (`audit_events`) |
| PR 2 | seasons.test.ts (20, 21, including the three guard trigger cells); player-registrations.test.ts (1, 2, 6, 8, 9); players.test.ts rewrite; audit.test.ts extensions (11, 12, 13, 25, the `player_history` cells); boards.test.ts extension (23); signup loop additions (`player_registrations`, `seasons`) |
| PR 3 | board picker eligibility unit tests (22); no new database boundary |
| PR 4 | export.test.ts (4, 18); export builder unit tests |
| PR 5 | import.test.ts (5, 7, 8, 13, 14, 15, 16, 17, 19, plus the namesake and failed-batch cells and non archived season import); namesake fixture pair; parser unit tests; signup loop addition (`import_batches`) |
| PR 6 | renew.test.ts (`renew_registrations` gate, non archived target season, idempotency, renewal audit); deno tests for the season aware Spond import (5 Spond arm; T17 pins re-asserted; namesake skip limitation pinned) |
...
- Relying on UI filters for access: rejected; access is never a view filter. Read is club wide via `players.view`, and team is a UI and data filter only, never an RLS boundary. The rejected team scoped read design is recorded in docs/security/registered-players-boundary.md.
...
- The security suite grows from one players file (seven cells) to six new files plus extensions, on the order of sixty new cells, and its runtime joins CI (Docker pulls for `supabase start`; the matrix doc's caution about reliability before making the job a required check applies).
...
- Read is club wide via `players.view`, so no team scoped RLS is introduced and the 0016 standing rule is preserved; `member_teams` and `all_teams` stay UI filter drivers, not access controls, so no new access relevant role is added to the test surface. The club wide read cells (coach reads the whole register, parent and outsider read nothing) are explicit.
...
2. Coach team scope: recommended, club wide read via `players.view` (team is a filter, the 0016 standing rule preserved; team scoped read is the rejected alternative, docs/security/registered-players-boundary.md). Shapes T1, T18 and the players and registrations matrices.
...
8. Audit retention: APPROVED policy (PR 2), child linked audit events retained indefinitely at current scale, reviewed annually, no legal period claimed; verified subject erasure runs as a controlled administrative process covering the player linked audit identifiers, `safe_changes` and backups; no automatic deletion or purge is added (T16, T22).
...
15. `audit.view` versus a separate per player history path: RESOLVED, `player_history` gated on `audit.view` (managers and admins hold it by default; coaches holding only `players.view` do not see per player history), chosen so historical child linked audit records are not exposed to coaches by default; the club wide Activity page also requires `audit.view`; adds read cells to audit.test.ts. The per player History UI is deferred; only the gated database read path is built.
...
- Harness: global setup extensions (manager fixture, `coachView` fixture, the namesake fixture pair, club B `audit.view` grant, quarantine loop additions) land with the first file that needs them; `tests/security/local-grants.sql` gains the revoke restatement for `log_audit_event` alongside the existing `board_tokens_without_names` line.
- Dependencies: the SheetJS `xlsx` dependency is added in PR 4 (the export PR, which ships XLSX export and the XLSX template and evaluates the dependency there) and reused by the PR 5 import parser, per docs/roadmaps/registered-players-delivery-plan.md and docs/product/registered-players-import-export.md; no dependency is added by this scoping work.
