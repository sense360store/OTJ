...
**tests/security/import.test.ts** (PR 5). Properties 5, 7, 8, 13 (import arm), 14, 15, 16, 17 (server arm), 19; season validation (archived or foreign season refused); the in body team scope cells (docs/security/registered-players-boundary.md section 10; docs/adr/ADR-0007-player-import-export-architecture.md Consequences): the team scoped importer fixture committing a row whose team is outside its member team set aborts the whole transaction with zero rows written, and a blank team (Unassigned) row from the same fixture is refused because Unassigned requires `all_teams`; permission revocation cell (serviceClient removes `players.import` from the fixture role between calls, next call refused, T11); batch id cross club replay refused (T14); `import_batches` readable only per the boundary contract.

**tests/security/renew.test.ts** (PR 6). The `renew_registrations` RPC per docs/roadmaps/registered-players-delivery-plan.md PR 6: the capability gate (`players.manage` required: manager passes; parent, coach and outsider refused with no rows written); a source season outside the club refused; the same in body D4 scope arm as `import_players` (the team scoped importer fixture, granted `players.manage`, refused for a target row on a team outside its member team set and for an Unassigned row without `all_teams`, whole transaction aborted); idempotency per (player, season) on a repeated call; audit events carrying source `renewal` and one shared batch id, committed with the copied rows.
...
| PR 5 | import.test.ts (5, 7, 8, 13, 14, 15, 16, 17, 19, plus the in body team scope cells); team scoped importer fixture; parser unit tests; signup loop addition (`import_batches`) |
| PR 6 | renew.test.ts (`renew_registrations` gate, in body team scope, idempotency, renewal audit); deno tests for the season aware Spond import (5 Spond arm; T17 pins re-asserted) |
