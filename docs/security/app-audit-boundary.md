...
- Access to child linked events stays tightly capability gated: the club wide Activity page requires `audit.view`; per player history requires `players.view`; parents hold neither and read zero rows. The Activity page must respect this classification and never widen who can read child linked rows.
- Retention of child linked events is a child personal data decision requiring explicit approval (Retention, below). It is not an operational convenience and is not decoupled from child data retention.
...
- `player_registrations` UPDATE, by precedence, one event per semantic change: status becoming `withdrawn` writes `player.withdrawn`; status leaving `withdrawn` writes `player.restored`; any other status change writes `player.status_changed`; a `team_id` change writes `player.team_changed`; any other audited field change (for example `shirt_number` or `registered_date` alone) writes `player.registration_updated`. A single UPDATE that changes several columns emits the specific action per semantic change (a status and team change in one UPDATE emits both `player.status_changed` and `player.team_changed`), with the withdrawn and restored specialisations taking precedence over the generic status change. Whatever actions are chosen, every changed audited field still appears in `changed_fields` and `safe_changes`.
...
- Per player: an RPC or view `player_history(p_player_id)` gated on `players.view`, so a coach sees the history of any club player (read is club wide) without holding `audit.view`. Exact semantics live in docs/security/registered-players-boundary.md; presentation lives in docs/product/registered-players-ux.md.
...
Retention is a product decision requiring approval (unresolved decision 8), and for child linked events it is a CHILD personal data retention decision, not merely operational (Data classification, above). No legal retention period is claimed anywhere, because none has been evidenced. The recommended operational default: retain audit events indefinitely at current scale, reviewed annually. At this club's scale (one club, five teams, roughly 75 to 125 players a season) audit volume is low thousands of events per season; indefinite retention costs almost nothing and preserves accountability. Because child linked events are pseudonymous child personal data, the annual review explicitly reconsiders whether indefinite retention of that history remains proportionate, and any erasure obligation (above) is honoured regardless of the standing retention default.
...
- Legal deletion requests: child linked audit events are pseudonymous child personal data (Data classification, above), so a subject deletion request is NOT satisfied by the `players` and registrations deletion path alone. It must consider the audit identifiers (`entity_id`, `player_id`) and `safe_changes` for that child, and backups. The recommended handling, requiring approval alongside the retention decision: on a substantiated erasure request, the child linked audit rows for that entity are addressed explicitly (deletion or irreversible severing of the identifier), and the obligation is recorded as re appliable after any restore. Removing the current player row is necessary but not sufficient. The rows carry no names, which limits exposure, but the stable identifier and attribute history are still personal data.
...
- A single read path requiring `audit.view` for per player history, the alternative to unresolved decision 15. Rejected. It forces a bad choice: either coaches are denied the history of players they can already see (making History useless to its main audience), or `audit.view` is granted club wide to coaches, a grant far broader than the need that also opens the whole Activity feed. The separate `player_history` path gives each audience exactly its scope: coaches read any club player's history under `players.view`, and only `audit.view` holders see the club wide feed.
...
- Decision 8: audit retention. Recommended default: retain indefinitely at current scale, reviewed annually; no legal period claimed. Child linked events are pseudonymous child personal data, so a subject erasure request is handled explicitly against the audit identifiers and `safe_changes` and backups, not by the `players` deletion path alone (Retention and Data classification, above).
- Decision 15: `audit.view` versus per player history access. Recommended default: separate paths; `player_history` gated on `players.view` (club wide read), the club wide Activity page gated on `audit.view`.

Related decisions owned by sibling documents that shape audit reads: the import commit semantics (docs/adr/ADR-0007-player-import-export-architecture.md) determine which batch outcomes can exist. Player read access is club wide (decision 2, docs/security/registered-players-boundary.md), so `player_history` returns any club player's history to a `players.view` holder.
