...
### Data classification: child linked audit events are pseudonymous child personal data

This is a corrected classification and it governs the whole document. An audit event whose `entity_type` is `player` (player events anchor to the stable identity, so registration changes surface as player events) is CHILD-LINKED and is pseudonymous child personal data, even though it carries no name. It holds a stable `player_id` in `entity_id` that identifies one child within the club, and `safe_changes` may hold that child's team, season, status, shirt number and registered date history, plus `team_id` and `season_id`. A stable identifier tied to a specific child, together with attribute history, is personal data under any data protection reading; removing the name does not remove the linkage, it only removes the direct identifier. Season events (`entity_type` `season`), export events and batch summaries are not child linked and are operational data.

Consequences, applied throughout this document:

- Access to child linked events stays tightly capability gated: the club wide Activity page requires `audit.view`, and per player history also requires `audit.view` (coaches holding only `players.view` do not see per player history by default); parents hold neither capability and read zero rows. The Activity page must respect this classification and never widen who can read child linked rows.
- Retention of child linked events is a child personal data decision, now settled as approved policy (Retention, below). It is not an operational convenience and is not decoupled from child data retention.
- Deletion and subject access handling must consider the audit identifiers and `safe_changes`, not only the current `players` and `player_registrations` rows. Removing the current player row does NOT by itself prove every privacy obligation is met, because child linked audit rows (and backups) still hold pseudonymous history keyed to that child.
- Backups and restored copies contain this data; a restore can resurrect child linked audit rows, and any subject deletion actioned since the restore point must be re applied afterward.

What is still barred, unchanged: child NAMES and complete row snapshots never appear in `audit_events`, in any field. `safe_changes` is limited to the approved id and scalar list; a display name change records the field name only ("Player name corrected"), never the old or new value. The classification above is about the stable id and the attribute history the log legitimately holds, not about names, which it never holds.
...
Proposed action mapping (implementers need this exact). Distinct business actions are used so the Activity feed reads accurately from the action alone, without reinterpreting a misleading action name through the `source` or `season_id` fields:

- `players` identity INSERT: `player.created`, a new stable child identity, carrying the new `id` and no name in the payload.
- `player_registrations` INSERT: `player.registration_created`, a registration added for a season (manual add, spreadsheet import new row, Spond import new row), except that when the transaction local source context is `renewal` the trigger writes `player.renewed` instead, so a bulk renewal into a new season reads as "Renewed into 2027/28" rather than a bare registration creation. The Add player flow (identity plus registration, atomic) therefore writes BOTH `player.created` and `player.registration_created` in its one transaction. The identity insert and the registration insert must commit together in one transaction (in practice a single transactional add RPC, since two PostgREST requests cannot share a transaction); docs/security/registered-players-boundary.md makes that atomic paired write mandatory, so a registration can never exist without its identity and neither can be created unaudited.
- `players` UPDATE: `player.updated` (this is where the display name correction rule applies, below; the identity row's only routinely edited field is `display_name`).
- `players` DELETE: `player.deleted`, carrying `old.id` and no name. The event and the deletion commit together in one transaction.
- `player_registrations` UPDATE: one semantic event per UPDATE, chosen by single winner precedence `player.withdrawn` > `player.restored` > `player.status_changed` > `player.team_changed` > `player.registration_updated`. Status becoming `withdrawn` writes `player.withdrawn`; status leaving `withdrawn` writes `player.restored`; any other status change writes `player.status_changed`; a `team_id` change writes `player.team_changed` only when no higher precedence status transition occurred in the same UPDATE; any other audited field change (for example `shirt_number` or `registered_date` alone) writes `player.registration_updated`. A single UPDATE that changes several columns therefore emits exactly one event, the highest precedence match (a status and team change in one UPDATE emits `player.status_changed` alone, the team change still recorded in `changed_fields` and `safe_changes`). This refines the earlier multi event wording per the PR 2 decision. Every changed audited field still appears in `changed_fields` and `safe_changes`.
- `player_registrations` DELETE: when the parent identity no longer exists (the cascade from `player.deleted`), no separate event; the identity deletion covers it. When the parent survives (a data repair only; no product flow deletes a registration alone, Withdraw is the product operation), `player.registration_updated` with metadata noting the registration removal.
...
- A failed or rolled back action produces no success event and no per row events: no success event ever exists for work that did not commit. An RLS refusal, a constraint violation or a validation abort leaves no player, registration or season event in `audit_events`, and attempted action logging is out of scope. One deliberate, narrow exception exists for imports: a failed `import_players` attempt rolls back every business write and every per row event inside a PL/pgSQL exception subtransaction, while its batch bookkeeping (the `import_batches` row moved to state `failed` plus the `players.import_failed` summary) commits from the outer function, which sits outside the rolled back savepoint. This shape is fixed in this scoping PR (docs/adr/ADR-0007-player-import-export-architecture.md, Import failure bookkeeping) under its two constraints (no partial player data; no success event for uncommitted work), so a failed attempt is traceable and idempotent without any success record (see Import audit).
...
Every event row carries a `club_id` derived server side from the changed row or the actor's own club. The select policy scopes reads to `club_id = public.my_club()` and `has_perm('audit.view')`; a member of another club, or an outsider, receives zero rows for every query shape. The per player history path re checks club and the `audit.view` capability in its own gate; there is no team scope arm, because audit read access is club wide (docs/security/registered-players-boundary.md). No read or write path crosses clubs; the security test plan proves the outsider fixture reads nothing (docs/security/registered-players-threat-model.md).
...
Two separate access paths, by design (decision 15, resolved to `audit.view` for both, the safer direction):
...
- Per player: an RPC or view `player_history(p_player_id)` gated on `audit.view`, so only `audit.view` holders (managers and admins) read a player's history; coaches holding only `players.view` do not see per player history by default. The per player History UI is deferred, so only the gated database read path exists in PR 2. Exact semantics live in docs/security/registered-players-boundary.md; presentation lives in docs/product/registered-players-ux.md.
...
| Action | Raised by |
|---|---|
| `player.created` | identity INSERT trigger (a new stable child identity) |
| `player.registration_created` | registration INSERT trigger (manual, csv_import, xlsx_import, spond_import) |
| `player.renewed` | registration INSERT trigger when the source context is `renewal` |
| `player.updated` | identity UPDATE trigger (display name correction) |
| `player.registration_updated` | registration UPDATE trigger (fields with no more specific action, for example shirt number or registered date) |
...
Realised actions: `content_share.created`, `content_share.refreshed`, `content_share.rotated`, `content_share.revoked`, `content_share.invalidated`. As of Content Sharing PR 2 (`0039_public_share_read.sql`) `content_share.expired` is emitted by the service role `content_share_expiry_cleanup` function (a system event, `reason_code: expired_cleanup`, no actor) when it physically clears a share expired beyond the retention window; the scheduled invocation of that function is a gated deploy step with a named owner, so on hosted the action is registered and callable but only fires once that schedule is wired.
...
- Per row events (`player.created`, `player.registration_created`, `player.registration_updated`, and so on) from the triggers, each carrying the batch id via the `otj.audit_batch` GUC, so row level history links to its batch.

A failed attempt has a smaller, different shape, per the sanctioned exception in the same transaction guarantee above: every business write and every per row event rolls back inside the PL/pgSQL exception subtransaction so no partial player data survives, while the `import_batches` row (state `failed`, with a safe failure summary) and the `players.import_failed` summary event (source `csv_import` or `xlsx_import`, matching the attempt's format) commit from the outer function. This is fixed in docs/adr/ADR-0007-player-import-export-architecture.md (Import failure bookkeeping) under its two constraints (no partial player data; no success event for uncommitted work). That committed failure record also makes a duplicate Confirm of a failed batch return the stored failure rather than re running it, and a concurrent call with the same batch id blocks on the batch row lock and then replays the committed result.

The batch counts are exactly ADR-0007's column list: `rows_received`, `added`, `updated`, `resolved_new` (rows the user chose to Import as new from a name collision), `skipped` and `invalid`. Provenance per count: `rows_received` is the number of rows submitted to the RPC; `added`, `updated` and `resolved_new` are re derived server side and never taken from the preview; `skipped` and `invalid` are the withheld and rejected totals the server itself computed while validating. No count is trusted from the preview. The preview's warnings and blank row numbers exist only in the client preview and results screen and never enter the batch record or any audit metadata.

The `import_batches` row carries NO file fingerprint. The browser parses the file and sends only parsed rows to the RPC (docs/adr/ADR-0007-player-import-export-architecture.md), so the server never receives the original bytes and cannot verify any client declared hash; a client declared hash would carry no integrity value and is not stored. The batch UUID is the idempotency key and needs no fingerprint. If a future operational need for one is demonstrated, any retained fingerprint must be server derived from a canonical representation of the payload the server actually received, and must be treated as pseudonymous child personal data under the classification above; a plain client provided hash is not acceptable.

Never retained by the import audit, or anywhere server side: any file fingerprint, the original filename (filenames can contain personal information), the original file, player names, complete rows, raw validation messages containing personal data, formulas, contact details. The rejected row report is generated client side and never uploaded (docs/product/registered-players-import-export.md).
...
Retention is an APPROVED policy (PR 2), and for child linked events it is a CHILD personal data retention decision, not merely operational (Data classification, above). No legal retention period is claimed anywhere, because none has been evidenced. The approved policy: retain audit events indefinitely at current scale, reviewed annually. At this club's scale (one club, five teams, roughly 75 to 125 players a season) audit volume is low thousands of events per season; indefinite retention costs almost nothing and preserves accountability. Because child linked events are pseudonymous child personal data, the annual review explicitly reconsiders whether indefinite retention of that history remains proportionate, and a verified subject erasure request is honoured through a controlled administrative process covering the player linked audit identifiers, `safe_changes` and the relevant backup implications, regardless of the standing retention default. No automatic deletion or purge behaviour is implemented in PR 2.
...
- Legal deletion requests: child linked audit events are pseudonymous child personal data (Data classification, above), so a subject deletion request is NOT satisfied by the `players` and registrations deletion path alone. It must consider the audit identifiers (`entity_id`, `player_id`) and `safe_changes` for that child, and backups. The approved handling (PR 2): on a verified erasure request, the child linked audit rows for that entity are addressed explicitly through a controlled administrative process (deletion or irreversible severing of the identifier), and the obligation is recorded as re appliable after any restore. Removing the current player row is necessary but not sufficient. The rows carry no names, which limits exposure, but the stable identifier and attribute history are still personal data.
- Actor display name snapshots: adult operational data, retained for accountability.
- Old player identifiers: retained after deletion they identify no one from outside, but inside the club they remain a pseudonymous link to a specific former child and are treated as personal data for erasure and access purposes (Data classification, above).
...
- Fixed retention with scheduled pruning, the alternative to the retention decision. Rejected; the approved policy is indefinite retention at current scale, reviewed annually. At this club's scale (low thousands of events per season) a pruning job is new moving machinery whose failure modes (silent non pruning, over pruning) cost more than the storage it saves, and because child linked events are pseudonymous child personal data, their erasure is handled through the controlled administrative process on a verified request, not a blanket schedule. The annual review is the check that this reasoning still holds; if volume or the data profile changes materially, a fixed period becomes a small follow up decision, not a redesign.
- Requiring `audit.view` for per player history. Adopted, resolving decision 15 in the safer direction. Gating per player history on `audit.view` keeps historical child linked audit records from being exposed to coaches by default: only managers and admins, who hold `audit.view`, read a player's history, and the same capability governs the club wide Activity feed. The accepted trade off is that coaches holding only `players.view` do not see per player history by default; the club wide read of current registered players is unchanged. The per player History UI is deferred, so only the gated database read path exists in PR 2.
...
The numbered decisions from the canonical decision list that belong to this document, with their status. Decision 7 still carries a recommended default awaiting approval by the decision owners; decisions 8 and 15 are now settled as recorded below:
...
- Decision 8: audit retention. APPROVED (PR 2): retain indefinitely at current scale, reviewed annually; no legal period claimed. Child linked events are pseudonymous child personal data, so a verified subject erasure request is handled through a controlled administrative process against the audit identifiers, `safe_changes` and backup implications, not by the `players` deletion path alone (Retention and Data classification, above).
- Decision 15: `audit.view` versus per player history access. RESOLVED to `audit.view` (the safer direction): `player_history` is gated on `audit.view`, the same capability as the club wide Activity page, so per player history is not exposed to coaches (`players.view` only) by default.

Related decisions owned by sibling documents that shape audit reads: the import commit semantics (docs/adr/ADR-0007-player-import-export-architecture.md) determine which batch outcomes can exist. Player read access is club wide (decision 2, docs/security/registered-players-boundary.md), but per player history is gated on `audit.view`, so `player_history` returns any club player's history only to an `audit.view` holder, not to a coach holding `players.view` alone.
...
- `player_history` gate semantics (`audit.view` holders only): docs/security/registered-players-boundary.md.
