...
together on the next run, `read-content-share` FIRST (the old read function's
deny list refuses the new key, so a management function deployed ahead of it
would mint links that read as unavailable until the second deploy), and the
readback of each must be checked against the merged source. Two things are
deliberately true about that run:
...
In that order, read first, and the workflow runs them in that order too. Each
function bundles its own copy of `_shared/share.ts`, and the read function's
validator is the stricter reader of what the management function writes, so a
management function deployed ahead of the read function can mint links the old
read function refuses (DRILL-02b's `diagram` key is the concrete case). The
reverse order has no such window: the new read function accepts whatever the
previous management function wrote.
...
- the row is the unique newest one, recorded at the newest version, with
  exactly one row carrying the name `venue_layouts` (`assert_post` compares
  `target_rows` against 1). The ledger's **total** row count is not asserted:
  `assert_post` reads `rows_total` only to print it in its report table, so a
  ledger with any number of older rows passes. That count sits under the
  readback below, where it belongs;
...
- and, through the five registered object probes in `reviewed_migrations.py`:
  ...
  cast, for the reason 0049's review found), and no more than that: the probe
  neither evaluates the predicate nor checks that `venue_layouts_zones_shape`
  invokes it, so it does **not** establish that a four zone value on a five
  slot row is unrepresentable. That rests on the migration's own
  self-verification, which drives the predicate through the canonical layouts
  and some fifty refused values inside the apply transaction, and on review of
  the SQL; and that `audit_venue_layouts()` is
...
- the table grants are exactly what was reviewed: `authenticated` holds DELETE,
  ...
  table's;
- the ledger holds 48 rows in total. Read back only, for the reason above:
  nothing in the gate compares it.
...
0051 added one nullable integer column, `public.teams.sort_order`, one partial
unique index, `teams_sort_order_unique`, and `sort_order` on the allow list of
the existing `public.audit_teams()`; it wrote no row. Its apply evidence was
established for its own reconciliation: one `statements` entry hashing to
`a69fb87b31007eabd009bcab27aacecd`, `created_by` naming the workflow at commit
`855b0dc9fb0adfd7fa8e47cffe930638c2bf5759` on the COACH-1A branch, the
idempotency key `otj:migration:0051_team_sort_order`, and the index definition
`CREATE UNIQUE INDEX teams_sort_order_unique ON public.teams USING btree
(club_id, sort_order) WHERE (sort_order IS NOT NULL)` read back whole, which a
two column partial probe could not fix. That definition is re-read above rather
than carried from here, because 0052 is written around that index and had to
leave it untouched. The rest evidences that apply and nothing since; its row
remains the one immediately before the current head.
...
All of it evidences that apply and nothing since; its row is now two before the
current head.
...
evidences that apply and nothing since; its row is now three before the current
head.
...
The earlier superseded values are rejected by the same equality:
`20260823065041` (`0050_bulk_delete_players`, applied 2026-08-23 as the bulk
player deletion functions), `20260817104226`
...
`20260809184949` and `20260809081118`. A test pins every one of them, 0051's
value included.
