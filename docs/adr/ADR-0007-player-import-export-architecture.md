...
- The existing Modal `dismissible` prop (PR #103) is reused for the non dismissible import confirm, and the new Modal dialog and focus baseline plus the preview accessibility work in `docs/product/registered-players-ux.md` must land with or before the import confirm flow.
