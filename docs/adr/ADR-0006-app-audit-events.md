...
Assessed and excluded: an actor role or capability snapshot per event. Every audited action is already capability gated at commit time, the product pages have no read use for a per event grants list, and the column can be added later without disturbing anything. Also excluded: request bodies, filenames (an import batch stores a sha256 fingerprint of the file bytes instead, per docs/adr/ADR-0007-player-import-export-architecture.md), tokens and secrets of any kind.
...
Events with no underlying row write (the export event `players.exported`, the import batch summaries `players.import_completed` and `players.import_failed`) are written by a private SECURITY DEFINER function `log_audit_event(...)` with EXECUTE revoked from `public`, `anon` and `authenticated` (the `0028` revoke pattern), callable only from other definer RPCs and the service role. The export RPC writes `players.exported` in the same transaction as the read it audits. A failed import commits no player rows and no success event; its only committed trace is the batch record and the `players.import_failed` summary, written after the business changes have been rolled back inside the RPC's error handling (mechanism in docs/adr/ADR-0007-player-import-export-architecture.md).
...
Per player history is a separate, narrower read path: an RPC or view `player_history(player_id)` gated on `players.view` and the team scope of docs/security/registered-players-boundary.md, so a coach can see their own players' history without holding club-wide `audit.view`. The club-wide Activity page requires `audit.view`. Product behaviour for both surfaces is specified in docs/product/registered-players-spec.md and docs/product/registered-players-ux.md.

### Action catalogue and what is never logged

The initial catalogue covers Registered Players and seasons: `player.created`, `player.updated`, `player.team_changed`, `player.status_changed`, `player.withdrawn`, `player.restored`, `player.deleted`; `players.import_completed`, `players.import_failed`, `players.exported`, `players.spond_imported`; `season.created`, `season.updated`, `season.activated`, `season.archived`. `players.import_started` is deliberately not written: the batch row itself records initiation, and a started event with no completed event adds noise without operational value. Namespaces for future rollout (user.*, team.*, content_share.*, spond.*, drill.*, template.*, programme.*, session.*) are reserved in the catalogue documented in docs/security/app-audit-boundary.md.
...
Deferred to product approval. The recommended operational default is to retain indefinitely at current scale (low thousands of events per season), reviewed annually. Audit rows carry no child names, so a subject deletion request is satisfied by the players and registrations deletion path plus the tombstone rule (a deleted player renders as "Deleted player"); the trail does not have to be purged to honour it. Actor name snapshots persist after profile deletion for accountability. No legal retention period is claimed; none has been evidenced.
...
- Parents never see audit data: no capability, no policy arm, and the RequireCap pattern renders nothing while capabilities load. Coaches see per player history for players they can see, and nothing club-wide unless granted `audit.view`.
- Audit rows are deliberately name free, so the trail can outlive the data it describes without extending child data retention. The cost is softer history copy ("Player name corrected" rather than the values), accepted as decision 7.
...
The following numbered decisions from the scoping brief belong to this document. Each is written up above as the recommended default and requires explicit approval.
...
- Decision 8: audit retention period. Recommended default: retain indefinitely at current scale, reviewed annually; no legal period claimed.
- Decision 15: club-wide `audit.view` versus per player history access. Recommended default: separate paths, so coaches get their players' history through `player_history(...)` without holding club-wide Activity access.
