...
Per player history is a separate, narrower read path: an RPC or view `player_history(player_id)` gated on `players.view` (read is club wide, no team arm; docs/security/registered-players-boundary.md), so a coach can see any club player's history without holding club-wide `audit.view`. A child linked history row is pseudonymous child personal data, so this gate is a real access control. The club-wide Activity page requires `audit.view`. Product behaviour for both surfaces is specified in docs/product/registered-players-spec.md and docs/product/registered-players-ux.md.
...
Deferred to product approval. The recommended operational default is to retain indefinitely at current scale (low thousands of events per season), reviewed annually. Audit rows carry no child names, but a child linked event (entity_type player) is pseudonymous child personal data: it holds a stable player_id and that child's attribute history, so retention is a CHILD personal data decision, not merely operational, and it is not decoupled from child data. A subject erasure request is therefore NOT satisfied by the players and registrations deletion path alone: it must also address the child linked audit identifiers and `safe_changes` (deletion or irreversible severing of the identifier), and backups, since a restore can resurrect them. Removing the current player row is necessary but not sufficient; the tombstone rule ("Deleted player") limits exposure but the stable identifier and attribute history remain personal data. Actor name snapshots persist after profile deletion for accountability (adult operational data). No legal retention period is claimed; none has been evidenced. The full classification is in docs/security/app-audit-boundary.md (Data classification).
...
- Parents never see audit data: no capability, no policy arm, and the RequireCap pattern renders nothing while capabilities load. Coaches see per player history for any club player (read is club wide), and nothing club-wide unless granted `audit.view`.
...
- Decision 8: audit retention period. Recommended default: retain indefinitely at current scale, reviewed annually; no legal period claimed. Child linked events are pseudonymous child personal data, so a subject erasure request is handled explicitly against the audit identifiers and `safe_changes` and backups, not by the players deletion path alone.
- Decision 15: club-wide `audit.view` versus per player history access. Recommended default: separate paths, so coaches read any club player's history through `player_history(...)` (club wide read) without holding club-wide Activity access.
