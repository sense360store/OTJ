...
Status: Proposed (draft for review; the schema lands with the provisional 0030 series seasons and registration migration, number to be confirmed against the live ledger at apply time)
Date: 2026-07-16
...
- **At most one current season at all times, exactly one after setup.** The upper bound (at most one) is enforced by a partial unique index on `(club_id) WHERE is_current`, for every writer. The lower bound (at least one, so exactly one) is provided by bootstrap: the initial season migration creates the one current season for the existing club, and any future club creation path must transactionally create an initial current season in the same transaction as the club row (there is no in app club creation flow today, so this is a requirement on any future bootstrap and on the local seed). A live club is therefore never in a committed state with zero current seasons; the only zero state is a club that has not run setup. This document does not claim an unconditional "exactly one" while allowing a club with none. A guard trigger on `seasons` (BEFORE UPDATE OR DELETE) keeps the invariant during operation: it refuses any update that clears `is_current` outside `activate_season` and refuses deleting a current season, raising P0001 per the harness refusal conventions. The RPC identifies itself to the trigger through a transaction local flag (`set_config`, the same mechanism the audit foundation uses for flow context in `docs/adr/ADR-0006-app-audit-events.md`). The index alone would not be enough: without the trigger, a direct `update seasons set is_current = false` through the ordinary update policy would leave a club with no current season.
...
- **Activation is an RPC** (`activate_season`), one transaction: clear `is_current` on the old row, set it on the new, write the `season.activated` audit event. Archiving the outgoing season is an explicit option on the call, never automatic. When the option is taken, the same transaction sets `archived_at` on the outgoing season (after its `is_current` is cleared) and writes `season.archived` alongside `season.activated`. When it is not taken, which is the proposed default, the outgoing season stays open: non current but still writable, so late corrections during the changeover period need no unarchive round trip, and it is archived later by an explicit archive on the non current season. The choosing control is an "Also archive" checkbox on the Make current confirmation, unticked by default, specified in `docs/product/registered-players-spec.md` (Season management surface) and `docs/product/registered-players-ux.md` (section 13); those documents and this bullet state the same default. APPROVAL REQUIRED on the option default, recorded with Decision 14 under Unresolved items. Activation never creates, copies or modifies any registration.
- **Archiving the current season alone is refused**, enforced by the same guard trigger: an update that sets `archived_at` while `is_current` is true raises P0001. A club therefore always has a current, unarchived season after setup. Archival happens through the activation option above or explicitly on a non current season.
...
- **Renewal** into a new season creates new registration rows against the same player identities. Recommended: an explicit bulk Renew action (`players.manage`) copying chosen registrations from a source season into the current season as status pending, team carried forward, shirt number carried forward, `registered_date` empty, run as one transactional RPC with a batch id and audit events (source `renewal`). Until Renew ships, the export and import round trip is the renewal mechanism, and it requires editing the exported file first: the Season column must be cleared or overtyped with the new season's name, because a stale season name is a row error under the import's Season cross check and the commit is all or nothing, and the Registration Status and Registered Date columns are cleared if renewals should start pending with no date, since the export carries last season's values. The step by step renewal procedure is documented in `docs/product/registered-players-import-export.md`. Renew ships late in the plan (the PR 6 window in `docs/roadmaps/registered-players-delivery-plan.md`). APPROVAL REQUIRED on the status reset to pending and the carry forward of team and shirt.
...
- Who can see Unassigned registrations is settled: read is club wide via `players.view`, so every viewer sees them (team is a filter, not an access boundary). The RLS is specified in `docs/security/registered-players-boundary.md`.
...
The split described under Proposal. The comparison that decided it, across the twelve assessment dimensions, each argued for both models:
...
**Adopt the split: a first class `seasons` table, `players` as the stable club level identity keeping every existing uuid, and `player_registrations` as the one row per child per season record, exactly as specified under Proposal, subject to the approvals listed under Unresolved items.**

This is decision D1 of the scoping brief, together with the seasons model of D2. The twelve dimension comparison above decides it, most heavily on the tactics board reference dimension and the name duplication dimension. The schema of record is the Proposal section of this document; the operational sequence that lands it is `docs/roadmaps/registered-players-delivery-plan.md`.
...
- **The players RLS is rewritten.** The select gate moves from `sessions.create` to the new `players.view` capability, writes to `players.manage`, both club scoped with no team arm (read is club wide; team is a filter, the 0016 standing rule preserved), and the insert policies pin `created_by = auth.uid()` explicitly (closing the confirmed 0021 comment versus clause gap). The exact policy semantics, the capability catalogue growth from thirteen to twenty keys, the default grants and the deliberate coach write reduction are specified in `docs/security/registered-players-boundary.md` and are approval gated there.
...
- **No new access precedent.** Read is club wide via `players.view`, so the standing rule recorded in `supabase/migrations/0016_member_teams.sql` ("teams scope no row level security") is preserved and no team scoped RLS is introduced. The rejected team scoped design and its four reasons are recorded under Alternatives in `docs/security/registered-players-boundary.md`.
...
- **Decision 10: season renewal mechanism.** Recommended: an explicit bulk Renew action into a chosen non archived season (defaulting to the current one) as status pending, with team and shirt number carried forward and `registered_date` empty; the export and import round trip, following the documented renewal procedure in `docs/product/registered-players-import-export.md` (Season column cleared, status and date columns cleared for renewal semantics), is the day one alternative and works into any non archived season since the spreadsheet import is no longer current-season-only. Within this: approval on the status reset to pending and on the carry forward of team and shirt.
- **Decision 14: archived season absoluteness.** Recommended: archived seasons are read only for registrations, enforced in the database, with unarchive (gated `seasons.manage`, audited) as the only escape hatch. Within this: the activation archive option's default. Recommended: the option is not taken by default, so activating a successor leaves the outgoing season open and it is archived later by the explicit Archive action; the Make current dialog's "Also archive" checkbox, unticked by default, is the choosing control.
...
- **RLS and policy work** exactly as specified in `docs/security/registered-players-boundary.md`, including the club wide capability gated policies (no team scope, 0016 preserved), the `created_by` pin and the parent role's continued total exclusion.
...
- **Security tests and threat coverage** per `docs/security/registered-players-threat-model.md`, mapped to phases in `docs/roadmaps/registered-players-delivery-plan.md`, including refusal cells in `tests/security/seasons.test.ts` for the seasons guard trigger: clearing `is_current` directly, archiving the current season and deleting a current season all refuse with P0001.
