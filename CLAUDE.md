...
- Spond RSVP is context. Attendance is the coach's own record in `register_entries`, which nothing in the Spond pipeline reads, writes, defaults or constrains. Going and not present, declined and present, and no reply and present are all valid. The register must be fully usable with no Spond configuration at all, and a Spond failure must render as no context, never as "nobody is coming".
