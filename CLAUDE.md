...
- The fixture rule, `isFixtureTitle`, is the narrowest thing that catches those three. All of it must hold at once: the label splits on ONE versus separator (an en dash, em dash or hyphen with whitespace both sides, or `v`/`vs`/`versus`) into exactly two sides; one whole side IS one of the club's team names, not a word inside a longer side; and BOTH sides read as names rather than prose, meaning at most four words each, every word capitalised, numeric or a small joining word. The third clause is what keeps "Titans – passing and receiving" in the Training view and is the reason the rule may hide anything at all. It deliberately misses "U8 v Horbury", "Titans U9 – Hepworth" and "titans – rastrick", each of which is one line to widen, because every miss shows a fixture and every false catch hides a training night. Without the club's teams in hand the rule cannot fire and the classifier is exactly what it was before it existed.
