...
The `QueryClient` is module level (`src/main.tsx`) and lives for the lifetime of the tab, so the browser cache outlives a sign-out. Postgres RLS is still the only enforcement of who may read what; this is about what a screen renders before its own read answers.

- **A change of signed in user id drops the cache.** `src/lib/queryIdentity.ts` is the whole rule and `hooks/useAuth.tsx` is the only caller, on the line above the `setSession` that renders the incoming member's screens. Same handler, one line earlier, so the ordering is structural rather than a race between two listeners on the same Supabase event. Any change to it is a gated migration of the auth flow, the same as the guard itself.
- **The rule is about the cache, never about keys.** Almost every read key in `queries.ts` is a bare global literal (`['teams']`, `['drills']`, `['sessions']`), and TanStack serves cached data first, so a second member signing in on the same tab was handed the first member's club. Club scoping the keys was the alternative and is weaker three ways: it stops the previous identity's rows being served without removing them from the tab, it needs a club id that is itself an async read so every key would re-key mid flight through a shared `undefined`, and it is a rule roughly fifty existing read families and every future one have to remember, where forgetting is silent and presents exactly like the defect. Do not add an identity or club segment to a query key expecting it to be the boundary.
- **Reset AND remove, because neither alone is sufficient**, which is measured rather than assumed. `removeQueries` (and `queryClient.clear()`, which is the same call) empties the cache and leaves a mounted observer still reporting the previous identity's rows as a success, which is the entire defect surviving on the path where nothing unmounts: an invite or recovery link opened in a tab somebody else is signed into emits `SIGNED_IN` with no `SIGNED_OUT` before it. `resetQueries` empties the observer, keeps the entry, and refetches every active query at once, so a sign out fires that burst with no JWT to carry it. Resetting each query and then removing it does both halves and starts nothing: a mounted screen drops to pending in the same tick, and React's next commit, which the reset provokes by notifying, starts the new identity's read. Reset also destroys the query, so a read still in flight for the previous identity is cancelled and cannot write its rows back after the drop.
- **The default is drop.** `ANONYMOUS_QUERY_FAMILIES` is a closed list holding exactly one entry, `public-share`, whose route mounts outside the auth guard and reads an anonymous snapshot addressed by a share id and a secret in the URL. Everything else, including a read family written tomorrow, is identity bound without anybody remembering to opt it in. Widening that list is a deliberate act somebody reviews.
- **Only a change drops.** A token refresh, a user update and a repeated initial session all report the same user id and drop nothing, so the cache is not blown hourly. The first observation counts as a change and is a no-op on an empty cache, because treating boot as a special case would be one more thing to get wrong.
...
- `src/lib/queryIdentity.test.ts` is the behavioural half and opens with the leak reproduced against a bare client, so the guard beside it cannot go vacuous. `src/lib/queryIdentity.invariant.test.ts` is the tripwire on the wiring and names in its own tests the shapes it cannot catch.
