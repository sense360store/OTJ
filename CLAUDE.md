...
- `src/lib/tonight.invariant.test.ts` pins the rules that would be quietly expensive to lose: one card on session day, no persistence outside Save, one implementation of the filters, the bib rule staying in `bibs.ts`, and no user visible Register wording.
