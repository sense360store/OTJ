...
- The linking screen's **setup diagnostics** answer why a registered player has no candidate at all, which was three different Spond side problems presenting as one unexplained list. From the SAME `groups/` response the candidates come from, `spond-link-members` returns the members of that parent group the team's mappings do not reach, as their own closed shape `{ display_name, spond_member_id, subgroup_ids }` beside the unchanged `LinkCandidate`, so the screen can say `In Spond · no team assigned`, `In Spond · assigned to another team` or `Not found in Spond group data`. No second Spond call is made, staff are excluded before any row is emitted, and nothing new is persisted. It fails closed both ways: an unproved scan states nothing rather than claiming a match or an absence, and a name carried by more than one member is ambiguous rather than any category implying identity. "Not found in Spond group data" is deliberately not "not in Spond": absence from one parent group is not absence from Spond.
- **The diagnostic row carries the member id, and that replaced a narrower rule with a stronger one.** It carried none while the screen could only describe a mismatch; the team reconciliation below resolves an already linked child BY MEMBER ID, which is the whole reason it is not a name match, and an unlinked child cannot be bound to a member that has no id to bind. What replaces "a diagnostic row carries no member id" is a rule about the action rather than the payload: nothing links and nothing moves from such a row without an explicit human press on a row already proved unambiguous on both sides, and the database refuses an unlinked child outright. An id the links table would refuse arrives as `''`, which reads as no identity and offers no action. `LinkCandidate` was deliberately NOT widened alongside it: `subgroup_ids` there would only say whether a member of this team's subgroup is also in another team's, and the answer is "offer nothing" either way.
- **Spond is the source of truth for a child's current team, and `spond_reconcile_player_team` (0049) is the only way OTJ agrees with it.** A durable link is what a move rides on and the only thing it may ride on: the RPC refuses a child with no `player_spond_links` row, so "never move anybody on a name match" is a database rule rather than a screen's convention. It must also be the SAME link, NAMED: "this child has a link" is not proof, because between the scan and the press another manager can unlink the member the destination was derived from and correctly link a different one whose Spond team is elsewhere, with the child's OTJ team unmoved, so neither the link check nor the expected-team check would notice. A proved call names `p_expected_member_id` and a repointed link is `stale_link`; exactly one of that and `p_confirm_member_id` must be supplied, so a null never means "any link will do". That is the case `planRosterImport`'s cross team guard already refuses and explicitly defers to a durable link for; that guard is unchanged. A proved child moves on one press. An unlinked child moves only after a human confirms which Spond member they are, which creates the link and moves the team in ONE transaction. `matched_by` is `'suggested'`, which is what it has always meant, and there is still no `'auto'`. Everything ambiguous offers nothing and says which ambiguity it is: more than one mapped team's subgroup, a subgroup TWO teams both claim, a subgroup no team maps (never read as Unassigned, which would take a child out of a squad because a mapping is missing), two Spond members of one name, two registered children of one name in the club, an unproved scan, and a club where any team maps a whole Spond group rather than a subgroup. A contested subgroup is carried as evidence rather than deleted from the resolution map, and it is checked FIRST so it outranks any unique mapping beside it: deleting it made it indistinguishable from a subgroup nobody maps, and those are opposite facts, since nobody claiming a subgroup cannot be a second team while two teams claiming it is a second team that cannot be named. Both readers share one predicate and the diagnostic gets its own sentence rather than borrowing the one about two people of one name. The rules are pure in `src/lib/spondReconcile.ts`, which composes `spondSetupRows` rather than re-deriving the ambiguity, so there is one name matcher on the screen. Only the CURRENT season moves, because the RPC derives the season and the caller cannot name one; no player identity is created, no link is deleted or repointed, no register entry, session or Spond mirror row is touched, and nothing is written toward Spond. Concurrency is a per (club, player) then per (club, member) advisory lock in that fixed order, a canonically ordered `FOR UPDATE` over the link rows, `FOR UPDATE` on the registration and an optimistic expected-team check, so a row somebody else moved is refused and a repeat press is an idempotent no-op. The member key covers what a row lock structurally cannot, a member with no link row yet, so two confirmations of the same free member return `member_linked_elsewhere` rather than a raw unique violation. Those locks bind only callers of this function, and the ordinary linking screen still inserts into `player_spond_links` directly from Accept and from Choose holding none of them, so the confirmation insert also carries a `unique_violation` handler: it re-reads ownership after the winner has committed and returns `member_linked_elsewhere` or `player_linked_elsewhere`, never a raw `23505`, never a move after losing the identity race, and never a repointed or deleted link. Fixed at the RPC boundary rather than by requiring every future direct insert to join a lock protocol, which is the assumption that would rot. Audited by the existing triggers only (`player.team_changed`, `player.spond_linked`) sharing one batch id per press, with no new audit source value. "Apply all safe Spond changes" is hidden outright while any row still needs an identity settled.
...
- **The aggregate is a labelled sentence, and never a split.** Naming the population was not enough: the planner card, the Plan from Spond rows and the event picker each rendered the four counts as four figures beside four words, and "20 accepted, 24 declined" is read as a statement about the club's players however it is captioned, because that is what a going figure means on a football app. On the 11 August event those two figures were an audience of 50 people, while the covered squad was 10 going and 14 not going out of 27 linked children from 40. Both pairs were correct. So the four counts are never four figures beside four words, on any surface. There is no exempt surface. The admin Synced events card was one for a day, on the reasoning that inspecting the mirror means seeing what was synced, and that reasoning was wrong in the only way that matters: that card is the screen the defect was reported from, so exempting it left the reported screen unchanged. `SPOND_COUNT_LABELS` no longer exists and `tonight.invariant.test.ts` fails the build if a count field off an event row reaches any file but `src/lib/spond.ts`.
- **Whether the sentence carries the event's reply figure is a narrower question, and hiding it everywhere was half a fix.** `spondAudienceNote` reads `Spond audience: 50 people invited`, a headcount with no reply word, and that is what the picker, the planner card and the admin mirror show. None of them sets a player figure against the AUDIENCE: the picker and the planner card print no player figure at all, and the admin mirror's `Linked players (27) · 10 going` counts every linked child who replied to the event rather than the children a session covers, which is a third population and not the comparison a coach is making. A going number on any of them therefore has nothing to be read against and is read as a squad. Players &amp; groups is the exception, because it is the one screen where a coach holds both figures at once. They read a going figure in Spond, meet a Going chip of 11, and a headcount of 50 answers neither: it is a third number, and the one they came in holding had been removed from the product entirely. So `spondAudienceReplyNote` renders there and only there, as `Spond audience: 50 people invited · 21 of them going`, with `tonightPlayersNote` directly beneath it in the same shape: `Players this session covers: 40 · 11 of them going`. **The reply figure exists only as half of that pair**, and the screen reaches the state where the other half is missing: a session whose coverage was never set counts nobody, so the players sentence says nothing rather than claim a squad of zero, and the aggregate falls back to the plain headcount rather than stand alone as a going number over an audience of fifty. The players sentence also withholds its own reply clause until the reply read has answered, and both sentences are absent entirely when no event is linked. "of them" is load bearing, tying each reply to the population its own sentence has already named, and the two sit one under the other because the comparison is the point. `tonight.invariant.test.ts` fails the build if the reply sentence reaches any other surface, if either half of the pair is dropped, or if the players sentence is composed anywhere but `src/lib/tonight.ts`.
- **A reply word is only ever rendered beside a population its own sentence has already named.** Going, No reply, Not going and Waiting describe children wherever they stand alone, and the one place a reply figure counts anybody else is the paired sentence on Players &amp; groups described below, where the audience is named in the same breath and the covered squad is stated directly underneath in the same shape. Admin Synced events shows them as `Linked players (27) · 10 going · 3 no reply · 14 not going · 0 waiting`, built by `countLinkedResponses` from `spond_event_responses` through the same `matchesResponse` predicate Tonight's chips use, so one predicate answers for both. That population is honestly a different one from Tonight's and wears a different label: it is every linked child who replied to one event, with no session and therefore no coverage to narrow by, whereas a Tonight chip counts the children a SESSION covers. Two populations, two labels, never two words for one. The read (`useSpondEventResponseCounts`) selects `spond_event_id` and `status` and no member id, so it cannot resolve to a child even in memory; a database without 0045, a read in flight and a failed read all render as silence rather than as zero, and an event no linked child replied to says so in words. `src/routes/adminSpondCounts.screens.test.tsx` renders the real screen over the 11 August figures and fails if an API reply word or an aggregate count reaches the markup.
...
What a session STARTS covering is a separate question with one answer, `newSessionCoverage` in the same module: the whole club, unless the thing the session was created FROM names one team. The signed in coach's profile team is deliberately not an input. It used to seed coverage on all three create paths, and it is the wrong kind of fact for the job: a profile team is a personal default about which team a coach mostly works with, used to focus a view and preselect a picker, while coverage is a statement about the night, and the two agree only by coincidence. A coach whose profile said Trojans opened New Session and found Trojans already ticked, took a club template and got a Trojans session, and planned the club's own Tuesday from Spond, an event stored with no team precisely because Spond had invited everybody, into a session covering one squad. Nothing on any of those screens said a choice had been made for them. Widening is the safe direction and the only one available: a coach who wants one team unticks the other four and can see what they are doing, while a coach who never noticed the narrowing had no such prompt, and team is a filter and a default rather than access control, so a wider default grants nobody anything. `blankSession` takes no team at all, so the argument cannot be filled with one again; the planner seeds the club once the team read answers, once, and only into a draft nobody has touched, so a refetch, a team added or a team deleted cannot rewrite an edited draft and an existing session never seeds at all. The two paths that save before the coach sees anything, Plan from Spond and Use template, wait for that read rather than passing an empty club through: `newSessionCoverage` refuses to guess for an unknown club, which is right, and a saved session covering nobody has a register listing nobody with no later repair, because the planner it opens will not re-seed a stored row. `src/lib/newSessionCoverage.invariant.test.ts` fails the build on a second implementation, on a create path reading `profile.team_id`, and on either wait being dropped.
