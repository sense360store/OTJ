...
Session day has ONE operational surface. The job is not attendance, it is organisation: who is expected, which children the coach is including, and what bib each needs so the squad splits into groups. `src/lib/tonight.ts` holds the whole model and `src/routes/SessionRegister.tsx` is a thin shell over it. The file name and the `/session-day/:id/register` segment keep the old word to avoid route churn; nothing user visible says Register.
...
- **`register_entries.present` is legacy naming.** Its product meaning is "included in tonight's groups". An audit at the time of the change proved nothing else depended on it meaning physical attendance: no export, no report, no parent view, no season stat, no audit trigger, no Edge Function. `marked_by`/`marked_at` are still trigger stamped and still true, since they record who last wrote the row. Renaming the column is deferred debt, not a pending correctness problem. If a real attendance feature is ever introduced it needs its own column, not this one.
...
- The precedence, in order: `status` of `completed` is authoritative and means past, even for a future date; a session being driven live is active however far past its expected end; otherwise the expected end is the local start plus the planned duration; the fallback duration applies where the plan cannot answer; `now` later than the expected end means past, and everything else, an unreadable date included, is active. Every rule that can hide is narrow, and every ambiguity resolves towards keeping the session on screen, the same asymmetry the training classifier uses.
