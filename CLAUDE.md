...
- The stored attendance surface is exact, the children's data boundary as amended by owner decision of record (`docs/adr/ADR-0008-training-day-logistics.md`; the authoritative statement is `docs/security/spond-data-boundary.md`). `spond_events` holds four integer counts per event (accepted, declined, unanswered, waiting) plus event facts, with no payload or member columns by design. Beyond the counts, the app may store only: opaque `spond_member_id`s in the player link table and per event response rows, the four response statuses per linked member, and the link between a `spond_member_id` and a `public.players` row. Names in the app resolve exclusively through `players.display_name` via that link. Everything else Spond knows about a member or guardian (names, emails, phone numbers, comments, addresses or any other payload fragment) remains forbidden to persist or log anywhere, and test fixtures are synthetic, never real payloads.
- Member names are read in at most two isolated places. `spond-roster-import` runs only when someone presses Import for a specific mapped team, never on a schedule and never as part of the attendance sync; from each member it takes only the child's full name and an optional shirt number, writing them to the `players` roster. The linking flow (arriving with the training day logistics queue) reads member display names and ids transiently, returned only to an authorised coach client to present match candidates, persisting and logging none of them. Neither reads a guardian, contact or any other member field, and both log only HTTP status and counts, never a name.
...
- Sync direction is Spond to app only. Sessions are arranged and answered in Spond; the Hub holds a synced copy of the counts and, for linked members, response statuses.
