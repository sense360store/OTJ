...
- FA assets enter the platform only when a signed-in coach imports a specific resource, by pasted URL or from the catalogue browser. The platform fetches FA pages only on a signed-in coach's action and never follows links beyond the single requested page, with one sanctioned exception: a programme overview's own week links may be followed, one level, same host, capped, as part of importing that one user-chosen resource.
- The catalogue browser indexes the FA sessions listing on demand. The index stores facts and links only: titles, summaries, the FA's taxonomy labels and the listing thumbnail's URL, hot-linked from the FA CDN and never re-hosted. Full assets are stored only when a specific resource is imported, unmodified, with attribution.
- Bulk import is selection-based and club-initiated: the coach ticks specific resources, the scale is stated plainly before the run, and the queue imports them one at a time through the same per-resource import. No syncing or importing is ever automatic or scheduled.
