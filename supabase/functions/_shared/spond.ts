...
// implements.
//
// THE CHILDREN'S DATA BOUNDARY, the rule that shapes this module, as
// amended by owner decision of record (docs/adr/ADR-0008 and the
// authoritative docs/security/spond-data-boundary.md). Spond event
// responses identify children and their parents: member ids in the
// response arrays, names elsewhere in the payload. The stored surface
// is exact: four integer counts per event, and per event response
// statuses for LINKED members only, an opaque member id plus one of the
// four states, nothing else. buildEventRow is the only place an event
// becomes a spond_events row and SPOND_EVENT_COLUMNS its complete key
// set; deriveMemberStatuses and buildResponseRows are the only place
// response rows come from and SPOND_RESPONSE_COLUMNS their complete key
// set, filtered against the club's link table so an unlinked id (a
// guardian, an adult member, an unlinked child) is counted into the
// aggregates and discarded exactly as before the amendment. The event's
// recipients object, which embeds member names, is never read at all;
// subgroup scoping is delegated to Spond's own subGroupId filter (see
// eventsQuery). Names are read transiently in exactly one flow, the
// linking candidates (reduceLinkCandidate below), returned only to an
// authorised client and never persisted or logged.
//
...
// =====================================================================
// Per event responses for linked members (the ADR-0008 amendment).
//
// The sync stores one row per (event, linked member): the opaque member
// id and one of Spond's four response states. The filter against the
// club's player_spond_links rows is the boundary's linked only rule:
// ids without a link never become rows, so guardian and adult member
// ids stay out of the tables entirely. The id arrays are the only part
// of the responses object read, the same arrays deriveCounts measures;
// the recipients object is never read.
// =====================================================================

// The token shape migration 0045 enforces on spond_member_id: an opaque
// identifier, never a name or an email. An id failing this is discarded
// here so a row the database would refuse is never attempted.
export const SPOND_MEMBER_ID_PATTERN = /^[A-Za-z0-9_-]{1,64}$/

export const SPOND_RESPONSE_STATUSES = ['accepted', 'declined', 'unanswered', 'waiting'] as const
export type SpondResponseStatus = (typeof SPOND_RESPONSE_STATUSES)[number]

// Defensive cap on ids read from any one response array, mirroring the
// events and roster caps: a malformed or unexpectedly huge array is
// bounded, never iterated whole.
export const MAX_RESPONSE_IDS_PER_ARRAY = 1000

// The complete set of columns a response row may carry, mirroring
// migration 0045's spond_event_responses (id is generated by the
// database). The boundary test asserts buildResponseRows emits exactly
// these keys and only primitive values.
...
export interface SpondResponseRow {
  club_id: string
  // The spond_events row's uuid, not Spond's text event id: the rows
  // reference the synced event and cascade with it.
  spond_event_id: string
...
// The response arrays in a fixed order, each mapped to its stored state.
// waitinglistIds is Spond's spelling; unconfirmedIds is deliberately not
// here, exactly as it is not one of the four counts.
const RESPONSE_ARRAYS: ReadonlyArray<{ field: string; status: SpondResponseStatus }> = [
  { field: 'acceptedIds', status: 'accepted' },
  { field: 'declinedIds', status: 'declined' },
  { field: 'unansweredIds', status: 'unanswered' },
  { field: 'waitinglistIds', status: 'waiting' },
]

// One event's per member statuses for the linked members only. Reads the
// four id arrays and nothing else from the event; an id outside the
// linked set, or one failing the opaque token shape, is discarded. An id
// appearing in more than one array (a payload anomaly) keeps its first
// claim, in the fixed array order above, so the result is deterministic
// and unique per member.
export function deriveMemberStatuses(event: unknown, linked: ReadonlySet<string>): MemberStatus[] {
  if (linked.size === 0) return []
  const responses = asRecord(asRecord(event).responses)
  ...
  for (const { field, status } of RESPONSE_ARRAYS) {
    ...
    for (const id of value.slice(0, MAX_RESPONSE_IDS_PER_ARRAY)) {
      ...
      if (!linked.has(id) || !SPOND_MEMBER_ID_PATTERN.test(id)) continue
      seen.add(id)
      out.push({ spond_member_id: id, status })
    }
  }
  return out
}

// Statuses shaped into the rows the upsert writes, once the synced
// event's database id is known.
...
// spond-sync. The attendance pipeline never touches a name (its stored
// surface is counts plus linked member ids and statuses, per the
// amended boundary above); names are read in exactly two isolated
// places, this roster reduction and the linking candidates section
// below, both pinned by their test files.
//
...
// =====================================================================
// Linking candidates: members to name and id pairs, transiently.
//
// Used only by the spond-link-members Edge Function, the linking flow of
// the amended boundary (docs/security/spond-data-boundary.md). This is
// the one place the pipeline pairs a member's id with a name, and the
// pair exists only in the function's response to an authorised client:
// never persisted, never logged. The reduction reads exactly the member
// id and the same name fields the roster import reads, and discards
// everything else, guardians and contacts above all. Pinned by
// spond_link_test.ts.
// =====================================================================
...
// One member reduced to the linking pair, or null when it has no usable
// opaque id or no usable name. The id must fit the 0045 token shape, so
// a candidate the link table would refuse is never offered.
export function reduceLinkCandidate(member: unknown): LinkCandidate | null {
  const m = asRecord(member)
  const id = typeof m.id === 'string' ? m.id : ''
  if (!SPOND_MEMBER_ID_PATTERN.test(id)) return null
  const display_name = rosterDisplayName(member)
  if (!display_name) return null
  return { spond_member_id: id, display_name }
}

// The candidates from the scoped members of a team's mappings, de-duped
// by member id (a member in two mappings appears once), capped like the
// roster read. Order preserved from the input for a stable screen.
export function planLinkCandidates(members: unknown[]): LinkCandidate[] {
  const seen = new Set<string>()
  const out: LinkCandidate[] = []
  for (const member of members) {
    if (out.length >= MAX_ROSTER_MEMBERS) break
    const reduced = reduceLinkCandidate(member)
    if (!reduced || seen.has(reduced.spond_member_id)) continue
